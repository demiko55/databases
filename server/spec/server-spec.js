...
describe('Persistent Node Chat Server', () => {
  ...
  it('Should insert posted messages to the DB', (done) => {
    ...
    axios.post(`${API_URL}/users`, { username })
      ...
      .then(() => {
        ...
        const queryString = 'SELECT * FROM messages where username = "Valjean" and text = "In mercy\'s name, three days is all I need." and roomname = "Hello"';
        const queryArgs = [];
  ...
  it('Should output all messages from the DB', (done) => {
    ...
    const queryString = 'insert into messages (text, username, roomname, message_id) values ("' + text + '", "' + username + '", "' + roomname + '", "' + messageId + '")';
    const queryArgs = [];
    ...
    dbConnection.query(queryString, queryArgs, (err) => {
      ...
      axios.get(`${API_URL}/messages`)
        .then((response) => {
          ...
          expect(messageLog[messageLog.length - 1].text).toEqual(text);
